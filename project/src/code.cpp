...
inline void print_coloured(char* word, Colour* colour, uint8_t x, uint8_t y)
{
	gfx_SetTextXY(x + TEXT_WIDTH / 2, y);
	for (size_t i = 0; i < WORD_SIZE; i++) {
		gfx_SetTextBGColor(colour[i]);
		gfx_SetColor(colour[i]);
		gfx_Rectangle(x + i * TEXT_WIDTH, y, x + (i + 1) * TEXT_WIDTH, y + TEXT_HEIGHT);
		gfx_PrintChar(word[i]);
	}
}

inline void get_feedback(char* secret, char* guess, Colour* out)
{
	for (uint8_t i = 0; i < WORD_SIZE; i++)
		if (secret[i] == guess[i])
			out[i] = Colour::CORRECT;
		else
			out[i] = Colour::ABSENT;
	
	for (uint8_t i = 0; i < WORD_SIZE; i++)
		for (uint8_t j = 0; j < WORD_SIZE; j++)
			if (out[j] == Colour::ABSENT && out[i] != Colour::CORRECT && secret[i] == guess[j]) {
				out[j] =  Colour::PRESENT;
				break;
			}
}
...
{	
	...
	*/
	...
	if (theme == 1) {
		...
		gfx_palette[Colour::CORRECT]         = gfx_RGBTo1555(0x53, 0x8d, 0x4e);
		gfx_palette[Colour::ABSENT]          = gfx_RGBTo1555(0x3a, 0x3a, 0x3c);
		gfx_palette[Colour::PRESENT]         = gfx_RGBTo1555(0xb5, 0x9f, 0x3b);
		...
		gfx_palette[Colour::TEXT]            = gfx_RGBTo1555(0xff, 0xff, 0xff);
		gfx_palette[Colour::ABSENT]          = gfx_RGBTo1555(0xd3, 0xd6, 0xda);
		gfx_palette[Colour::PRESENT]         = gfx_RGBTo1555(0xc9, 0xb4, 0x58);
		gfx_palette[Colour::CORRECT]         = gfx_RGBTo1555(0x6a, 0xaa, 0x64);
...
inline void init_game()
{
	...
	gfx_SetMonospaceFont(TEXT_WIDTH);
...
int main() 
{
	...
	for (uint8_t i = 0; i < 6; i++) {
		...
		Colour hints[WORD_SIZE];
		get_feedback(secret_word, user_word, hints);

		gfx_SetTextXY(0, gfx_GetTextY());
		print_coloured(user_word, hints, 0, i * TEXT_HEIGHT);
